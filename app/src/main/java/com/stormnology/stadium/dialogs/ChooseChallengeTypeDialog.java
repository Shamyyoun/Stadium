...
import com.stormnology.stadium.controllers.ChallengeTypeController;
 ...
 */
public class ChooseChallengeTypeDialog extends ProgressDialog {
    private ChallengeTypeController typeController;
    private RecyclerView recyclerView;
    private Button btnSubmit;
    private RadioButtonsAdapter adapter;
    private List<ChallengeType> data;
    private OnCheckableSelectedListener itemSelectedListener;
    private int selectedItemId;

    public ChooseChallengeTypeDialog(final Context context) {
        super(context);
        setTitle(R.string.challenges_type);

        // create controllers
        typeController = new ChallengeTypeController();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            SerializableListWrapper<ChallengeType> dataWrapper = (SerializableListWrapper<ChallengeType>) savedInstanceState.getSerializable("dataWrapper");
            if (dataWrapper != null) {
    ...
    protected int getContentViewResId() {
        return R.layout.dialog_choose_challenge_type;
    }
    ...
    private void loadData() {
        ...
        // send request
        ConnectionHandler connectionHandler = ApiRequests.challengeTypes(context, this);
    ...
    public void onSuccess(Object response, int statusCode, String tag) {
        // get data
        ChallengeType[] typesArr = (ChallengeType[]) response;
        data = new ArrayList<>(Arrays.asList(typesArr));

        // check size
        if (data.size() == 0) {
            showEmpty(R.string.no_types_found);
        } else {
            updateUI();
        }
    }
    ...
    public void onFail(Exception ex, int statusCode, String tag) {
        showError(R.string.failed_loading_types);
    }
    ...
    private void selectCheckedItem() {
        int itemPosition = typeController.getItemPosition(data, selectedItemId);
        if (itemPosition != -1) {
